...
// Default nav links (same logic as Nav.tsx)
const DEFAULT_NAV: { href: string; label: string; slug: string; defaultVisible: boolean }[] = [
...
function getNavLinks(pagesConfig: PageConfig[] | null) {
  ...
  return DEFAULT_NAV
    .map(link => {
      const cfg = pagesConfig.find(p => p.slug === link.slug)
      return { ...link, visible: cfg ? cfg.visible : link.defaultVisible, order: cfg?.order ?? 99 }
    })
    .filter(l => l.visible)
...
export default function Footer({ tenant, config }: Props) {
  ...
  // Hide footer on property detail pages — on desktop it's a fixed full-screen layout
  // (mirrors reference HTML where detail overlay covers everything including footer)
  // On mobile the detail is scrollable so we keep the footer
  const isDetail = /^\/listings\/.+/.test(pathname ?? '')
  // We hide on all screen sizes; mobile detail has enough self-contained content
  ...
  return (
    <>
      ...
      <footer style={{
        background: '#f7f7f7',
        padding: 'clamp(28px, 4vw, 44px) clamp(16px, 4vw, 40px) 24px',
        display: 'grid',
        ...
        <div style={{ gridColumn: 'span 2' }}>
          {logoUrl ? (
            <img
              src={logoUrl}
              alt={tenant.name}
              style={{ height: 36, objectFit: 'contain', marginBottom: 10 }}
            />
          ) : (
            <div style={{
          ...
          {config?.hero_subtitle && (
            <p style={{ fontSize: 13, color: '#717171', lineHeight: 1.7, maxWidth: 260, margin: 0 }}>
              {config.hero_subtitle}
            </p>
          )}
          {/* Social icons */}
        ...
        {/* Navigation column */}
        <div>
          <h5 style={{ fontSize: 10, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#aaa', marginBottom: 12, fontWeight: 600 }}>
            Navegación
          </h5>
          {navLinks.map(link => (
            <Link key={link.slug} href={link.href} style={{
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
              {config?.contact_email && (
                <a href={`mailto:${config.contact_email}`} style={{ display: 'block', fontSize: 13, color: '#555', textDecoration: 'none' }}
                  onMouseEnter={e => (e.currentTarget.style.color = 'var(--accent, #f5a623)')}
                  onMouseLeave={e => (e.currentTarget.style.color = '#555')}
                >
                  {config.contact_email}
              ...
              {config?.whatsapp && (
                <a href={`https://wa.me/${config.whatsapp}`} target="_blank" rel="noopener noreferrer"
                  style={{ display: 'block', fontSize: 13, color: '#555', textDecoration: 'none' }}
                  onMouseEnter={e => (e.currentTarget.style.color = 'var(--accent, #f5a623)')}
                  onMouseLeave={e => (e.currentTarget.style.color = '#555')}
                >
                  +{config.whatsapp}
      ...
      <div style={{
        background: '#f7f7f7',
        padding: 'clamp(8px, 2vw, 13px) clamp(16px, 4vw, 40px)',
...
function SocialLink({ href, label, children }: { href: string; label: string; children: React.ReactNode }) {
  return (
    <a href={href} target="_blank" rel="noopener noreferrer" aria-label={label} style={{
      ...
      color: '#888', textDecoration: 'none', transition: 'color .2s, border-color .2s',
      background: '#fff',
    }}
